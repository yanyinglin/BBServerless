...
data_path = conf['BBServerless']['data_path']
...
def create_docker_file(path, name):
    # name = name.split("-")[0]
    name = name.lower()
    path += '{model_name}.Dockerfile'.format(model_name=name)
...
def create_function_dockerfile(models):
    for m in models:
        ...
        print("create_docker_file")
        create_invoker(func_abs_path, m)
        print("create_invoker")
...
def create_invoker(path, name):
    name = name.lower()
    path += 'invoke.sh'
    tag = "k.harbor.siat.ac.cn/kubeless/"+ name + ':' + version

    with open(shell_tpl, 'r') as tpl:
        tpl_script = tpl.readlines()
    
    tp = ''.join(tpl_script).format(model=name, tag=tag, func_name=name)
